...
- Output format (Markdown):
  ...
  2) Use a Markdown subheading: "#### Symptoms identified and information". On the next line, add one sentence introducing the section. Then include a Markdown table with columns: Symptom | Identified | Details. Use Yes/No in Identified. Keep Details as short prose sentences using the symptom order provided. After the table, add one sentence noting other screened symptoms were asked about but not reported.
  3) Use a Markdown subheading: "#### Likely diagnosis". On the next line, write a short paragraph (no bullet list). Include the lay term with the medical term in brackets for each diagnosis; separate multiple diagnoses with semicolons.
  4) Use a Markdown subheading: "#### Expectations for treatment or further investigation". Then use a hyphen bullet list (green boxes). When MRI is mandatory, explain it is to exclude other serious causes (do not name specific conditions).
