...
- Output format (Markdown):
  ...
  1) Use a Markdown subheading: "#### Symptoms identified and information". On the next line, add one sentence introducing the section. Then include a Markdown table with columns: Symptom | Question | Answer. Use the questionsAsked list to populate rows in the same order provided, using the question and answer text verbatim so the table matches the assessment review table. For repeated questions under the same symptom, leave the Symptom cell blank after the first row so the symptom label is not duplicated.
  2) Use a Markdown subheading: "#### Diagnosis". On the next line, write one short sentence that includes the phrase "Based on your answers to the questions" (patient) or "Based on the answers to the questions" (clinician) and names the most likely diagnosis (lay term with medical term in brackets). Do not mention other diagnoses or recommended steps.
  3) Use a Markdown subheading: "#### Alternative diagnoses". On the next line, write a short paragraph listing other possible diagnoses from alternateDiagnoses only (lay term with medical term in brackets). If alternateDiagnoses is empty, explicitly note that no other possible diagnoses were suggested by the logic. End this paragraph by stating that a proper consultation is required to confirm the diagnosis.
  ...
  5) Use a Markdown subheading: "#### Potential treatment options". Then use a hyphen bullet list derived from expectations (the most likely options from the branch logic). Use the expectation text verbatim.
