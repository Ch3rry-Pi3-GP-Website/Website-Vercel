...
import {
  ...
  computeNoseAssessment,
  noseSymptoms,
  type NoseAnswerEvent,
  type NoseAssessmentSummaryPayload,
  type NoseQuestionStep,
  type NoseResponses,
} from "@/lib/noseAssessment";

export type NoseAssessmentSession = {
  area: "nose";
...
export default function NoseAssessmentFlow({
  audience,
  onSessionChange,
}: NoseAssessmentFlowProps) {
  const [events, setEvents] = useState<NoseAnswerEvent[]>([]);

  const assessment = useMemo(
    () => computeNoseAssessment(events, audience),
    [events, audience]
  );
  ...
  }, [assessment, audience, events.length]);

  const screenedCount = useMemo(() => {
    return noseSymptoms.filter(
      (symptom) => session.responses[symptom.id].present !== null
    ).length;
  }, [session.responses]);

  const reviewRows = useMemo(() => {
    return session.summaryPayload?.questionsAsked ?? [];
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-[var(--color-muted)]">
            Epistaxis follows its own branch. If absent, facial pain, nasal
            obstruction, sneezing, running/discharge, and anosmia continue
            through the shared rhinopathy pathway.
          </p>
        </div>
        <div className="rounded-full border border-[var(--color-border)] bg-white/80 px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-[var(--color-ink)]">
          Symptoms screened: {screenedCount} / {noseSymptoms.length}
        </div>
      </div>
      ...
      {assessment.isComplete ? (
        <div className="mt-8 space-y-4">
          <div className="rounded-2xl border border-dashed border-[var(--color-border)] bg-[var(--color-shell)] p-6 text-sm text-[var(--color-muted)]">
            Assessment complete. Review your answers below before generating the
            report.
          </div>
          <div className="rounded-2xl border border-[var(--color-border)] bg-white/80 p-5 shadow-lg">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-2 text-sm text-[var(--color-muted)]">
                  Check your responses. If anything is incorrect, restart the
                  assessment.
                </p>
              </div>
              <button
